...
function FinalPage() {
  ...
  const [effectiveModelUrl, setEffectiveModelUrl] = useState(modelUrl || defaultModel);
  ...
  useEffect(() => {
    const fetchHotspots = async () => {
      try {
        const res = await fetch(`/api/annotations/by-collection/${encodeURIComponent(collectionName)}`);
        ...
        if (isUrl) {
          setEffectiveModelUrl({ glb: modelId, usdz: modelId.replace(/\.(glb|gltf)$/i, '.usdz') });
          return;
        }
        ...
        const check = async (url) => {
          try {
            ...
            return ct.includes('model/gltf-binary') || ct.includes('model/gltf+json') || ct.includes('application/octet-stream') || ct.includes('model/vnd.usdz');
          } catch (e) {
            return false;
          }
        };
        ...
        if (await check(inferredGlb)) {
          setEffectiveModelUrl({ glb: inferredGlb, usdz: inferredGlb.replace(/\.(glb|gltf)$/i, '.usdz') });
          return;
        }

        if (await check(altGlb)) {
          setEffectiveModelUrl({ glb: altGlb, usdz: altGlb.replace(/\.(glb|gltf)$/i, '.usdz') });
          return;
        }

        setModelCheckMessage(`Modell \"${filename}\" nicht gefunden oder ungültig. Zeige Standardmodell.`);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const onError = async (ev) => {
      try {
        console.error('model-viewer: error event', ev, 'src=', effectiveModelUrl);
        ...
        if (effectiveModelUrl && typeof effectiveModelUrl.glb === 'string') {
          try {
            ...
            console.warn('Inspected resource:', effectiveModelUrl.glb, 'status=', r.status, 'content-type=', ct);
            ...
            console.error('Error while fetching model for inspection:', fetchErr);
          }
        }
      } catch (e) {
  ...
  return (
    ...
    >
      ...
      <model-viewer
        ref={modelViewerRef}
        src={effectiveModelUrl?.glb || effectiveModelUrl?.gltf || defaultModel.glb}
